		...
		<view class="comfirm-list order-detail-list my-order-list">
			<view class="ul">
				<view class="li" v-for="item in confirmList" v-if="confirmList.length>0" @tap="toOrderDetail(item.orderId)">
					...
					<view class="total"><text class="ji">合计：<text class="sub">{{item.payPrice}}{{item.currency==1?"Love Bird币":"BTC"}}</text></text></view>
					...
					success: (res) => {
						if (res.data.code === 0) {
							...
							data.forEach((item, i) => {
								...
								item.currencyImg = item.currency == 1 ? '/static/ic_coin_big.png' : '/static/ic_btc_identification.png';

<template>
	<view class="wrap-content wrap">
		<sub-nav :title="title">
			<view class="text" @click="$toPage('/pages/user/withdrawCoinRecord?status=0')">记录</view>
		</sub-nav>
		<view>
			<view class="row b-b">
				<text class="tit">账户类型</text>
				<view class="input">币账户(Love Bird)</view>
			</view>
			<view class="row b-b">
...
<script>
	...
	export default {
		data() {
			return {
				accountType:"",//转账选传到后台的类型。coinType为2使用，accountType：1:Love Bird积分; 2:LB积分;3:Love Bird币; 4:LB币
				toUserName:"",
				toUserNumber:"",
				message: "",
				title: "提币",
		...
		methods: {
			...
			confirm() {
					...
					this.transformLoveBird()//币转成(Love Bird积分)
			}
		}
	}
</script>
